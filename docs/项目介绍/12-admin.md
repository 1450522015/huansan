...
- **战局管理**：查看所有 PK 战局，支持按状态筛选（等待中/战局中/已结束），战局中显示当前回合，已结束显示结束时间。已取消的战局不入库不展示（与 `GET /api/admin/battles` 对齐）。「复制调试」调用 `GET /api/admin/battles/:id/debug`，JSON 与手游战局页「复制调试信息」一致（见 [`10-http-api.md`](./10-http-api.md) §战局管理）。
