<template>
  <div class="calc-page">
    <header class="page-head">
      <div class="head-text">
        <h1 class="title">属性计算</h1>
        <p class="sub">输入配置 JSON，计算属性结果</p>
      </div>
    </header>
    ...
    <section class="calc-body card">
      <div class="calc-panels">
        <div class="panel">
          <label class="panel-label">配置 JSON</label>
          <textarea v-model="inputText" class="inp textarea" rows="20" placeholder="粘贴配置 JSON"></textarea>
        </div>
        <div class="panel-actions">
          <button class="btn primary" @click="onCalc" :disabled="calcing">{{ calcText }}</button>
        </div>
        <div class="panel">
          <label class="panel-label">属性 JSON</label>
...
import { 玩家配置转玩家属性 } from '@/core/attr.js'
...
function onCalc() {
  errMsg.value = ''
  outputText.value = ''
  if (!inputText.value.trim()) { errMsg.value = '请输入配置 JSON'; return }
  let config
  try {
    config = JSON.parse(inputText.value.trim())
  } catch {
    errMsg.value = '配置 JSON 格式错误，请检查'; return
  }
  calcing.value = true
  try {
    const result = 玩家配置转玩家属性(config)
